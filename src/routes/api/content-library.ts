      ...
      GET: async ({ request }) => {
        try {
          ...
          const { data: content, error: contentError } = await supabaseServer
            .from("content_items")
            .select("id,title,platform,published_at,external_source,external_url,created_at")
            .eq("user_id", user.id)
            .order("created_at", { ascending: false });
          if (contentError) throw contentError;
          const ids = (content ?? []).map((item) => item.id);
          const { data: nodes, error: nodeError } = ids.length
            ? await supabaseServer.from("dna_nodes").select("content_id,type").in("content_id", ids)
            : { data: [], error: null };
          ...
          return Response.json({
            items: (content ?? []).map((item) => ({
              id: item.id,
              title: item.title,
              platform: item.platform ?? (item.external_source === "youtube" ? "YouTube" : "Unknown"),
          ...
          return Response.json({ error: "Unable to load your content library." }, { status: 502 });
        }
      },
    },
  },
